    ...
    <form action={action} className="flex flex-col gap-5">
      ...
      {footerConfig.whatsapp && (
        <div className="border-t border-[var(--border)] pt-4">
          <a
            href={`https://wa.me/${footerConfig.whatsapp}?text=${encodeURIComponent(SOPORTE_WHATSAPP_MENSAJE)}`}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-center gap-2 rounded-lg border border-[#25D366]/40 bg-[#25D366]/10 px-4 py-2.5 text-sm font-medium text-[#25D366] transition-colors hover:bg-[#25D366]/15"
          >
            <svg
            ...
            ¿Problemas para acceder? Escribinos
          </a>
        </div>
      )}
    </form>
